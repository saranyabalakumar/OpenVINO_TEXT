...
# get_ipython().run_line_magic('pip', 'install -q "onnx" "onnxruntime" "einops" "transformers>=4.31.0"')
...
# 
# >**Note**: Currently INT8 weights decompression is supported only for CPU, GPU support will be available later. For running inference on GPU, we recommend using the FP16 model for optimal memory consumption, performance, and load time.
# 
# 
...
if "GPU" in device.value and int8_compressed_weights:
    warnings.warn("GPU device does not support weights compresstion, FP16 model will be used")
    int8_compressed_weights.value = False
